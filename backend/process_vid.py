...
async def process_job(job_id, video_path, overlays, job_dir, container_width, container_height, jobs, result_dir):
    ...
    try:
        ...
        if not text_overlays and not image_overlays:
            ...
            for overlay in image_overlays:
                ...
                w = int(w_container * scale_factor)
                h = int(h_container * scale_factor)
            ...
            cmd = ["ffmpeg"] + inputs + ["-filter_complex", ";".join(filter_complex), "-map", current_stream, "-map", "0:a?", "-c:a", "copy", "-y", output_path]
            ...
            if process_result.returncode != 0:
                ...
                print(f"what happend??: {error_msg}")
